...
public class QQIDSongSearchResult extends SearchResult<Song> {
    ...
    public QQIDSongSearchResult(List<String> IDs, boolean isMID) throws IOException {
        if (isMID) {
            for (String str : IDs) {
                songs.add(new QQSong(QQHTTPUtil.JSONHTTPRequest("http://c.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?songmid=" + str + "&format=json"), 0));
            }
        } else {
            for (String str : IDs) {
                if (!str.matches("^[0-9]*$")) {
                ...
                songs.add(new QQSong(QQHTTPUtil.JSONHTTPRequest("http://c.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?songid=" + str + "&format=json"), 0));
            }
        }
    }
